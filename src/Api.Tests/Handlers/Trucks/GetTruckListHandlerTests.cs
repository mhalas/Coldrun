...
{
    [TestFixture]
    public class GetTruckListHandlerTests
    {
        ...
        [Test]
        public async Task Handle_ShouldReturnsListOfTrucks()
        {
            ...
            using (var context = GetDbContext())
            {
                ...
                await context.Truck.AddRangeAsync(truckList, cancellationToken);
                await context.SaveChangesAsync(cancellationToken);

                var handler = new GetTruckListHandler(context);
                var result = await handler.Handle(new GetTruckListRequest(), cancellationToken);

                Assert.That(result, Is.EqualTo(truckList));
            }
        }
    }
}
