...
namespace Api.Tests.Handlers.Trucks
{
    ...
    public class GetTruckByIdHandlerTests
    {
        ...
        [Test]
        public async Task Handle_ShouldReturnsCorrectTruck_WhenPassTruckId()
        {
            ...
            using (var context = GetDbContext())
            {
                ...
                var handler = new GetTruckByIdHandler(context);
                var truck = await handler.Handle(new GetTruckByIdRequest()
                {
                    Id = 2
                }, cancellationToken);

                Assert.That(truck, Is.EqualTo(new Truck()
                {
                    Id = 2,
                    Code = "Code 2",
                    Description = "Description 2",
